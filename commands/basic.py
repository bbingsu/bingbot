...
IMG_URL = 'https://raw.githubusercontent.com/bbingsu/bingbot/main/image/'
...
async def 소개(ctx: Context):
    introduce.set_thumbnail(url=IMG_URL + random.choice(files))
...
@commands.command()
async def 랜덤(ctx: Context):
    embed = discord.Embed(title='',
                          description='')
    embed.set_image(url=IMG_URL + random.choice(files))
    embed.set_footer(text='먀옹')
    await ctx.channel.send(embed=embed)

basicCmd = [ 안녕, 바보, 기여, 이몸등장, 소개, 소개해봐, 자기소개, 랜덤 ]